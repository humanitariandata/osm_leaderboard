...
parser = argparse.ArgumentParser(description='Automatically get OSM Benin stats for APPs from geofabrik changesets.')
...
filename = 'benin_%s%s.json' % (start, end)
total_files = end - start + 1

print ("Fetching %s changesets from geofabrik...\n" % total_files)

os.system("node examples/cmd.js %s %s | jq -c '{type: .type, user:.user, changeset: .changeset, version: .version, timestamp: .timestamp}' > data/benin/%s" % (start, end, filename))
print ("Completed fetch...\n")

print("Converting result into valid JSON...")
os.system("./conv1.sh data/benin/%s" % filename)
