# combine stats from geojson.new files located in data/benin/.
...
def read_files(place="benin"):
	...
	fcheck = any (f.find ("benin") > -1 and f.endswith(".json.new") for f in files)
	if fcheck:
		json_files = [wd + i for i in files if i.endswith('.json.new')]
		print('Found %s geojson.new files in directory...' % len(json_files))
	else:
		sys.exit("make sure that json.new files exist in the data/benin directory.")
	return json_files
...
apus = pd.read_csv('data/benin/app_unames.csv')
...
if len(les_apps) > 0:
	...
	dis = pd.read_csv('data/benin/app_dnames.csv')
	...
	ap_us.sort('total_edits', ascending=True).tail(10)[['create', 'delete', 'modify']].iplot(filename="Benin Users Edits", title=name, xTitle='Edit Count', kind='barh', barmode='stack', margin=(200,50))
...
ts.iplot(filename='#MapBenin Timeline', title='#MapBenin Timeline', yTitle='Edit Count')
tsF = ts.iplot(filename='#MapBenin Timeline', title='#MapBenin Timeline', yTitle='Edit Count',asFigure=True)
...
py.iplot(tsF, filename='#MapBenin Timeline')
...
<head>
	<meta charset=utf-8 />
	<title>#MapBenin Key Stats</title>
	...
	<body>
		...
		<div class="row">
			<div class="small-12 columns">
				<h2>#MapBenin User Stats</h2>
				<p>Stats are collected from geofabrik changeset files. Use these graphs to track your progress in the <strong>#MapBenin</strong> mapping competition with the grand prize of a smartphone curtosy of <a href="https://www.mapillary.com/"><strong>Mapillary</strong></a>.</p>
			</div>
		</div>
		<div class="row show-for-small-only">
			<div class="small-12 columns">
				...
				<ul class="button-group">
				  ...
				  <li><a href="https://plot.ly/~rustyb/148/" class="button">#MapBenin Timeline Graph</a></li>
		...
		<div class="row ">
			<div class="small-12 large-6 columns">
				<strong>#MapBenin Leaderboard</strong>
				'''+ table + '''			
			</div>
			<div class="small-12 large-6 columns hide-for-small-only">
				...
				<strong>#MapBenin Timeline</strong>
				
				   <div>
    					<a href="https://plot.ly/~rustyb/148/" target="_blank" title="#MapBenin Timeline" style="display: block; text-align: center;"><img src="https://plot.ly/~rustyb/148.png" alt="#MapBenin Timeline" style="max-width: 100%;"  onerror="this.onerror=null;this.src='https://plot.ly/404.png';" /></a>
...
ap_us.sort('total_edits')[['create', 'delete', 'modify']].iplot(filename='test 1',kind='barh', barmode='stack', title='#MapBenin mappers by edits Feb 2015 to 18 July 2015',xTitle='Edits', yTitle='OSM Username')
ap_us.sort('total_edits')[['create', 'delete', 'modify']].iplot(filename='test 1',kind='barh', barmode='stack', title='#MapBenin mappers by edits Feb 2015 to 18 July 2015',xTitle='Edits', yTitle='OSM Username', colors=['rgba(31, 119, 180, 1)', 'rgba(255, 127, 14, 1)','rgba(44, 160, 44, 1)'], opacity='1',world_readable=True,bargap=0.0, width=0.0)

# get some stats for all the users contributing to map_Benin
...
pd.merge(yn[['number_changesets']].reset_index(), yc[['edit_count']].reset_index(), on='user').set_index('user').sort('edit_count', ascending=False)[0:10].plot(kind='barh', title="#MapBenin Top10 Users - Edits vs Changesets since Feb 2015")
