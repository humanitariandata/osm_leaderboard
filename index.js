...
function MetaUtil(opts) {
    ...
    this.baseURL = opts.baseURL || 'http://download.geofabrik.de/africa/benin-updates'; //'http://download.geofabrik.de/africa/benin-updates';
...
MetaUtil.prototype._read = function() {
    ...
    if (!this.started) {
        if (this.liveMode) {            //request.get('http://planet.osm.org/replication/changesets/state.yaml',
            request.get('http://download.geofabrik.de/africa/benin-updates/state.txt',
